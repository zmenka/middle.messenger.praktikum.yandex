...
class ChatApi extends BaseApi {
  ...
  list(offset: number = 0, limit: number = 100, title: string = '') {
    return this.httpTransport.get('', { queryParams:  { offset, limit, title}})
      .then((data: unknown) => data as Chat[]);
  }

  create(title: string) {
    return this.httpTransport.post('', { body:  { title }})
  ...
  setUsers(chatId: number, userIds: number[]) {
    return this.httpTransport.put('users/', {body: { chatId, users: userIds }})
  }

  deleteUsers(chatId: number, userIds: number[]) {
    return this.httpTransport.delete('users/', {body: { chatId, users: userIds }})
  }

  changeAvatar(chatId: number, data: FormData) {
    data.append('chatId', chatId.toString());
    
    return this.httpTransport.put('avatar', { body:  data})
    .then((data: unknown) => data as Chat);
  }
